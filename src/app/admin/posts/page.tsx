  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-zinc-100">게시글 관리</h1>
          <p className="text-zinc-400 text-sm mt-1">총 {posts.length}개</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-zinc-800 bg-zinc-800">
                ...
                <th className="text-left text-xs font-medium text-zinc-400 px-5 py-3 hidden md:table-cell">카테고리</th>
                <th className="text-left text-xs font-medium text-zinc-400 px-5 py-3 hidden md:table-cell">작성자</th>
            ...
            <tbody>
              {posts.map((post) => (
                <tr key={post.id} className="border-b border-zinc-800 hover:bg-zinc-800 transition-colors">
                  <td className="px-5 py-3.5">
                    <div>
                      <p className="font-medium text-zinc-200 text-sm">{post.title}</p>
                      <p className="text-xs text-zinc-500 truncate max-w-xs">{post.excerpt}</p>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Link
                        ...
                        className="p-1.5 text-zinc-500 hover:text-primary-400 rounded-lg hover:bg-zinc-800 transition-colors"
                      >
                        <Edit size={14} />
                      </Link>
