...
export default function TokensPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-extrabold text-white">초대 토큰 관리</h1>
        <p className="text-zinc-500 mt-1">회원가입에 필요한 초대 토큰을 생성하고 관리합니다.</p>
      </div>

      {/* 토큰 생성 */}
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-5 mb-6">
        <h2 className="font-bold text-white mb-4">새 토큰 발급</h2>
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 px-3.5 py-2.5 bg-zinc-800 border border-zinc-700 text-zinc-100 placeholder-zinc-600 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-600 focus:border-transparent"
          />
          <button
            onClick={createToken}
            disabled={creating}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-zinc-800 bg-zinc-800/50">
                <th className="text-left text-xs font-semibold text-zinc-500 px-5 py-3">토큰</th>
                <th className="text-left text-xs font-semibold text-zinc-500 px-5 py-3">메모</th>
                <th className="text-left text-xs font-semibold text-zinc-500 px-5 py-3">상태</th>
                <th className="text-left text-xs font-semibold text-zinc-500 px-5 py-3">발급일</th>
                  ...
                  <td className="px-5 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        t.used ? "bg-zinc-800 text-zinc-500" : "bg-green-950/60 text-green-400"
                      }`}
                    >
                      {t.used ? "사용됨" : "미사용"}
                    </span>
