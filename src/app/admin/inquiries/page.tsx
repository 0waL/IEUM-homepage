  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-white">문의 관리</h1>
      ...
      {inquiries.length === 0 ? (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-16 text-center">
          <MessageSquare className="mx-auto mb-3 text-zinc-700" size={32} />
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-zinc-800">
                ...
                <th className="text-left text-xs font-medium text-zinc-500 px-5 py-3 hidden sm:table-cell">작성자</th>
                <th className="text-left text-xs font-medium text-zinc-500 px-5 py-3 hidden md:table-cell">댓글</th>
                <th className="text-left text-xs font-medium text-zinc-500 px-5 py-3 hidden md:table-cell">작성일</th>
                <th className="text-left text-xs font-medium text-zinc-500 px-5 py-3">작업</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-800/50">
              {inquiries.map((inquiry) => (
                <tr key={inquiry.id} className="hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    ...
                    <p className="text-xs text-zinc-600 mt-0.5 sm:hidden">{inquiry.author}</p>
                  </td>
                  <td className="px-5 py-3 hidden sm:table-cell">
                    <p className="text-sm text-zinc-400">{inquiry.author}</p>
                    {inquiry.email && (
                      <p className="text-xs text-zinc-600">{inquiry.email}</p>
                    )}
                  </td>
